...
public class SumServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (a_raw == null || a_raw.isEmpty() || b_raw == null || b_raw.isEmpty()) {
                throw new NumberFormatException();
            } else {
                a = Double.parseDouble(a_raw);
                b = Double.parseDouble(b_raw);
                res = String.format("%.0f", (a + b));
                request.setAttribute("res", res);
            }
        } catch (NumberFormatException e) {
            request.setAttribute("error", "Invalid input! Try again");
        } finally {
            request.setAttribute("a", a_raw);
            request.setAttribute("b", b_raw);
            request.getRequestDispatcher("form.jsp").forward(request, response);
        }
    }
